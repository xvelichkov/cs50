...
int main(int argc, char* argv[])
{
    ...
    parse("GET  /index.html HTTP/1.1", NULL, NULL);
...
char* indexes(const char* path)
{
    char* path_index = (char*)malloc(strlen(path)+strlen("/index.php")+sizeof(char));
    if(path_index)
    {
    ...
    char* path_index_html = (char*)malloc(strlen(path)+strlen("/index.html")+sizeof(char));
    if(path_index_html)
    {
        strcpy(path_index, path);
        strcat(path_index_html, "/index.html");
                
        if(access(path_index_html, F_OK) == 0)
        {
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    size_t len = 0;
    *content = NULL;
    while ((n = read(fileno(file), buf, sizeof buf)) > 0)
    {
        *content = (char*)realloc(*content, len + n + 1);
        if(*content == NULL)
        {
            return false;
        }
...
const char* lookup(const char* path)
{
    if (path)
    {
        ...
        if (NULL != dot)
        {
            ...
            //skip dot and convert extension to lower case
            for (int i = 1, len = strlen(extension); i < len; i++)
            {
                extension[i] = tolower(dot[i]);
            }
...
bool parse_request_target(char* request_target, char* abs_path, char* query)
{
    bool result = false;
    ...
    if (strncmp(request_target, "/", 1) == 0)
    {
        ...
        {
            char* abs_path_token = strtok(request_target, "?");
            if (abs_path_token)
            {
                char* query_token = strtok(NULL, "");
                if(query_token)
                {
                    strcpy(query, query_token);
                }
            }
            else
            {
                abs_path_token = request_target;
            }
            strcpy(abs_path, abs_path_token);
            result = true;
            // printf("abs_path: %s\n", abs_path);
            // printf("query: %s\n", query);
        }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // very stupid workaround for two spaces
    if(strstr(request, "  ") == NULL)
    {
        ...
        {
            if(parse_request_target(request_target, abs_path, query))
            {
